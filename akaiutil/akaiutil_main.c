...
static void print_progressbar(u_int end, u_int now) {
...
#else
  PRINTF_ERR(
      "usage: %s [-h] [-r] [-F] [-C] [-l <lock-file>] [-o <start-offset>] [-s <pseudo-disk-size>] [-n <pseudo-disk-number>] [[-f] <disk-file> ...]\n",
      name);
  PRINTF_ERR("\t-h\tprint this info\n");
  PRINTF_ERR("\t-r\tread-only mode\n");
  PRINTF_ERR("\t-F\tdisable floppy filesystem\n");
  ...
  while ((op = getopt(argc, argv, OPT_STRING)) != EOF) {
    switch (op) {
      ...
      case 's':
        ...
        break;
      case 'n': pseudodisknum = (u_int) atoi(optarg);
...
#define CMDTOKMAX 16
    ...
    struct cmdtab_s {
      ...
      char *cmdhelp;
        ...
        {CMD_RESTARTKEEP, "restartkeep", 1, 1, "",
         "restart program (with opened disks, keep current directory path etc.)"},
        ...
        {CMD_SETOSVERI, "setosveri", 3, 3, "<file-index> <new-os-version>",
         "set OS version of file in S1000/S3000 volume"},
        {CMD_SETOSVERALL, "setosverall", 2, 2, "<new-os-version>",
         "set OS version of all files in current S1000/S3000 volume"},
        {CMD_SETUNCOMPRI, "setuncompri", 3, 3, "<file-index> <new-uncompr>",
         "set uncompr value of compressed file in S900 volume"},
        {CMD_UPDATEUNCOMPRI, "updateuncompri", 2, 2, "<file-index>",
         "update uncompr value of compressed file in S900 volume"},
        {CMD_UPDATEUNCOMPRALL, "updateuncomprall", 1, 1, "",
         "update uncompr value of all compressed files in current S900 volume"},
        {CMD_SAMPLE900UNCOMPR, "sample900uncompr", 2, 3, "<file-path> [<dest-vol-path>]",
         "uncompress S900 compressed sample file"},
        {CMD_SAMPLE900UNCOMPR, "s9uncompr", 2, 3, NULL, NULL},
        {CMD_SAMPLE900UNCOMPRI, "sample900uncompri", 2, 3, "<file-index> [<dest-vol-path>]",
         "uncompress S900 compressed sample file"},
        {CMD_SAMPLE900UNCOMPRI, "s9uncompri", 2, 3, NULL, NULL},
        {CMD_SAMPLE900UNCOMPRALL, "sample900uncomprall", 1, 2, "[<dest-vol-path>]",
         "uncompress all S900 compressed sample files in current volume"},
        {CMD_SAMPLE900UNCOMPRALL, "s9uncomprall", 1, 2, NULL, NULL},
        {CMD_SAMPLE900COMPR, "sample900compr", 2, 3, "<file-path> [<dest-vol-path>]",
         "compress S900 non-compressed sample file"},
        {CMD_SAMPLE900COMPR, "s9compr", 2, 3, NULL, NULL},
        {CMD_SAMPLE900COMPRI, "sample900compri", 2, 3, "<file-index> [<dest-vol-path>]",
         "compress S900 non-compressed sample file"},
        {CMD_SAMPLE900COMPRI, "s9compri", 2, 3, NULL, NULL},
        {CMD_SAMPLE900COMPRALL, "sample900comprall", 1, 2, "[<dest-vol-path>]",
         "compress all S900 non-compressed sample files in current volume"},
        ...
        {CMD_COPYVOL, "copyvol", 3, 3, "<akaiutil-volume-path> <new-volume-path>",
         "copy volume (with all of its files)"},
        {CMD_COPYVOL, "cpvol", 3, 3, NULL, NULL},
        {CMD_COPYVOLI, "copyvoli", 3, 3, "<akaiutil-volume-index> <new-volume-path>",
         "copy volume (with all of its files)"},
        {CMD_COPYVOLI, "cpvoli", 3, 3, NULL, NULL},
        {CMD_COPYPART, "copypart", 3, 3, "<akaiutil-partition-path> <dst-partition-path>",
         "copy all volumes of a partition"},
        {CMD_COPYPART, "cppart", 3, 3, NULL, NULL},
        {CMD_COPYTAGS, "copytags", 3, 3, "<akaiutil-partition-path> <dst-partition-path>",
         "copy all tags of a partition"},
        ...
        {CMD_FORMATFLOPPYL9, "formatfloppyl9", 1, 1, "",
         "format/erase low-density floppy for S900/S950, create filesystem"},
        {CMD_FORMATFLOPPYL9, "formatfl9", 1, 1, NULL, NULL},
        {CMD_FORMATFLOPPYL1, "formatfloppyl1", 1, 1, "",
         "format/erase low-density floppy for S1000, create filesystem"},
        {CMD_FORMATFLOPPYL1, "formatfl1", 1, 1, NULL, NULL},
        {CMD_FORMATFLOPPYL3, "formatfloppyl3", 1, 1, "",
         "format/erase low-density floppy for S3000, create filesystem"},
        {CMD_FORMATFLOPPYL3, "formatfl3", 1, 1, NULL, NULL},
        {CMD_FORMATFLOPPYH9, "formatfloppyh9", 1, 1, "",
         "format/erase high-density floppy for S950, create filesystem"},
        {CMD_FORMATFLOPPYH9, "formatfh9", 1, 1, NULL, NULL},
        {CMD_FORMATFLOPPYH1, "formatfloppyh1", 1, 1, "",
         "format/erase high-density floppy for S1000, create filesystem"},
        {CMD_FORMATFLOPPYH1, "formatfh1", 1, 1, NULL, NULL},
        {CMD_FORMATFLOPPYH3, "formatfloppyh3", 1, 1, "",
         "format/erase high-density floppy for S3000, create filesystem"},
        ...
        {CMD_FORMATHARDDISK9, "formatharddisk9", 1, 2, "[<total-size>[M]|SH205|SUPRA20M|SUPRA30M|SUPRA60M]",
         "create filesystem on harddisk for S900/S950 (size in blocks or MB, or drive name)"},
        ...
        {CMD_FORMATHARDDISK1, "formatharddisk1", 1, 3, "[<part-size>[M] [<total-size>[M]]]",
         "create filesystem on harddisk for S1000 (size in blocks or MB)"},
        ...
        {CMD_FORMATHARDDISK3, "formatharddisk3", 1, 3, "[<part-size>[M] [<total-size>[M]]]",
         "create filesystem on harddisk for S3000 or CD3000 (size in blocks or MB)"},
        ...
        {CMD_FORMATHARDDISK3CD, "formatharddisk3cd", 1, 3, "[<part-size>[M] [<total-size>[M]]]",
         "create filesystem on harddisk for CD3000 CD-ROM (size in blocks or MB)"},
        ...
        {CMD_WIPEPART3CD, "wipepart3cd", 1, 2, "[<partition-path>]",
         "create filesystem in harddisk partition for CD3000 CD-ROM"},
        ...
        {CMD_SCANBADBLKSPART, "scanbadblkspart", 1, 2, "[<partition-path>]",
         "scan for bad blocks/clusters in partition"},
        {CMD_MARKBADBLKSPART, "markbadblkspart", 1, 2, "[<partition-path>]",
         "mark free bad blocks/clusters in partition"},
        {CMD_SCANBADBLKSDISK, "scanbadblksdisk", 1, 2, "[<disk-path>]",
         "scan for bad blocks/clusters in all partitions of disk"},
        {CMD_MARKBADBLKSDISK, "markbadblksdisk", 1, 2, "[<disk-path>]",
         "mark free bad blocks/clusters in all partitions of disk"},
        ...
        {CMD_SETLNUMI, "setlnumi", 3, 3, "<volume-index> <new-load-number>",
         "set load number of volume (OFF for none)"},
        ...
        {CMD_GETPARAMI, "getparami", 3, 3, "<volume-index> <file-name>",
         "get parameters from volume (to external file)"},
        ...
        {CMD_PUTPARAMI, "putparami", 3, 3, "<volume-index> <file-name>",
         "put parameters to volume (from external file)"},
        ...
        {CMD_PUT, "put", 2, 3, "[<volume-path>/]<file-name> [<file-index>]",
         "put file (from external, use * for all local files)"},
        {CMD_PUT, "import", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE, "wav2sample", 2, 3, "[<volume-path>/]<wav-file> [<file-index>]",
         "convert external WAV file into sample file (use *.wav for all local WAV files)"},
        {CMD_WAV2SAMPLE, "wav2s", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE, "putwav", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE9, "wav2sample9", 2, 3, "[<volume-path>/]<wav-file> [<file-index>]",
         "convert external WAV file into S900 non-compressed sample file (use *.wav for all local WAV files)"},
        {CMD_WAV2SAMPLE9, "wav2s9", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE9, "putwav9", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE9C, "wav2sample9c", 2, 3, "[<volume-path>/]<wav-file> [<file-index>]",
         "convert external WAV file into S900 compressed sample file (use *.wav for all local WAV files)"},
        {CMD_WAV2SAMPLE9C, "wav2s9c", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE9C, "putwav9c", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE1, "wav2sample1", 2, 3, "[<volume-path>/]<wav-file> [<file-index>]",
         "convert external WAV file into S1000 sample file (use *.wav for all local WAV files)"},
        {CMD_WAV2SAMPLE1, "wav2s1", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE1, "putwav1", 2, 3, NULL, NULL},
        {CMD_WAV2SAMPLE3, "wav2sample3", 2, 3, "[<volume-path>/]<wav-file> [<file-index>]",
         "convert external WAV file into S3000 sample file (use *.wav for all local WAV files)"},
        ...
        {CMD_TPUT, "tput", 2, 2, "<take-file>",
         "put DD take (from external, use *"AKAI_DDTAKE_FNAMEEND" for all local DD take files)"},
        {CMD_TPUT, "timport", 2, 2, NULL, NULL},
        {CMD_WAV2TAKE, "wav2take", 2, 2, "<wav-file>",
         "convert external WAV file into DD take (use *.wav for all local WAV files)"},
        ...
        {CMD_TARXWAV9, "tarxwav9", 2, 2, "<tar-file>",
         "tar x in current directory (from external) with WAV conversion to S900 compressed sample"},
        {CMD_TARXWAV9, "tarputwav9", 2, 2, NULL, NULL},
        {CMD_TARXWAV9, "puttarwav9", 2, 2, NULL, NULL},
        {CMD_TARXWAV9C, "tarxwav9c", 2, 2, "<tar-file>",
         "tar x in current directory (from external) with WAV conversion to S900 compressed sample"},
        {CMD_TARXWAV9C, "tarputwav9c", 2, 2, NULL, NULL},
        {CMD_TARXWAV9C, "puttarwav9c", 2, 2, NULL, NULL},
        {CMD_TARXWAV1, "tarxwav1", 2, 2, "<tar-file>",
         "tar x in current directory (from external) with WAV conversion to S1000 sample"},
        {CMD_TARXWAV1, "tarputwav1", 2, 2, NULL, NULL},
        {CMD_TARXWAV1, "puttarwav1", 2, 2, NULL, NULL},
        {CMD_TARXWAV3, "tarxwav3", 2, 2, "<tar-file>",
         "tar x in current directory (from external) with WAV conversion to S3000 sample"},
        ...
        {CMD_MKVOLI1, "mkvoli1", 2, 4, "<volume-index> [<volume-name> [<load-number>]]",
         "create new volume for S1000 at index"},
        {CMD_MKVOLI1, "mkdiri1", 2, 4, NULL, NULL},
        {CMD_MKVOLI3, "mkvoli3", 2, 4, "<volume-index> [<volume-name> [<load-number>]]",
         "create new volume for S3000 or CD3000 at index"},
        {CMD_MKVOLI3, "mkdiri3", 2, 4, NULL, NULL},
        {CMD_MKVOLI3CD, "mkvoli3cd", 2, 4, "<volume-index> [<volume-name> [<load-number>]]",
         "create new volume for CD3000 CD-ROM at index"},
    ...
    for (;;) {
      ...
      /* execute command */
      switch (cmdnr) {
        case CMD_EXIT: goto main_exit;
        case CMD_RESTART:
        case CMD_RESTARTKEEP: FLUSH_ALL;
        ...
        case CMD_CDI: {
          ...
          break;
        case CMD_DIR: save_curdir(0); /* 0: no modifications */
          if (cmdtoknr >= 2) {
            if (change_curdir(cmdtok[1], 0, NULL, 1) < 0) { /* NULL,1: check last */
              PRINTF_ERR("directory not found\n");
              restore_curdir();
        ...
        case CMD_LCD:
          ...
          break;
        case CMD_LDIR: LDIR; /* XXX no error check */
          break;
        case CMD_LSFAT:
        ...
        case CMD_SAMPLE900COMPRALL: {
          ...
          for (fi = 0; fi < curvolp->fimax; fi++) {
            ...
            if (cmdnr == CMD_SAMPLE900UNCOMPRALL) {
              if ((tmpfile.type != AKAI_SAMPLE900_FTYPE)
                  || (tmpfile.osver == 0)) { /* not S900 compressed sample file? */
                continue; /* next */
              }
              PRINTF_OUT("uncompressing \"%s\"\n", tmpfile.name);
              FLUSH_ALL;
              ...
              if ((tmpfile.type != AKAI_SAMPLE900_FTYPE)
                  || (tmpfile.osver != 0)) { /* not S900 non-compressed sample file? */
                continue; /* next */
              }
              PRINTF_OUT("compressing \"%s\"\n", tmpfile.name);
              FLUSH_ALL;
        ...
        case CMD_FORMATFLOPPYH3: {
          ...
          switch (cmdnr) {
            case CMD_FORMATFLOPPYL9: lodensflag = 1;
              s3000flag = 0;
              s900flag = 1;
              break;
            case CMD_FORMATFLOPPYL1: lodensflag = 1;
              s3000flag = 0;
              s900flag = 0;
              break;
            case CMD_FORMATFLOPPYL3: lodensflag = 1;
              s3000flag = 1;
              s900flag = 0;
              break;
            case CMD_FORMATFLOPPYH9: lodensflag = 0;
              s3000flag = 0;
              s900flag = 1;
              break;
            case CMD_FORMATFLOPPYH1: lodensflag = 0;
              s3000flag = 0;
              s900flag = 0;
              break;
            case CMD_FORMATFLOPPYH3:
            default: lodensflag = 0;
              s3000flag = 1;
              s900flag = 0;
              break;
          }
        ...
        case CMD_PUTPART: {
          ...
          for (blk = 0; blk < curpartp->bsize; blk++) {
            ...
            if ((curpartp->type == PART_TYPE_HD)
                && (curpartp->index == 0) /* first partition on S1000/S3000 harddisk? */
        ...
        case CMD_PUTTAGS: {
          ...
          if (size
              != (4 + AKAI_PARTHEAD_TAGNUM * AKAI_NAME_LEN)) { /* invalid file size? (Note: tags magic and tag names) */
...
#endif
            wavname = NULL;
            if (akai_sample2wav(&tmpfile, -1, NULL, &wavname, SAMPLE2WAV_ALL)
                != 0) { /* error or not valid sample file? */
          ...
          for (;;) {
            if (allflag) {
...
#if 1
              {
                ...
                if (ascii2akai_filename(name, anamebuf, &osver, curvolp->type == AKAI_VOL_TYPE_S900)
                    == AKAI_FTYPE_FREE) {
        ...
        case CMD_UNLOCK:
          ...
          break;
        case CMD_PLAYWAV: PLAYWAV_STOP; /* stop playback of current external WAV file (if currently running) */
          if (cmdtoknr >= 2) {
            /* given WAV file name */
            /* check external WAV file name (and modify given file name if required) */
            /* Note: string in cmdtok[1] may be modified here */
          ...
          break;
        case CMD_STOPWAV: PLAYWAV_STOP; /* stop playback of current external WAV file (if currently running) */
          break;
        case CMD_DELWAV: PLAYWAV_STOP; /* stop playback of current external WAV file (if currently running) */
          if (curwavname[0] != '\0') {
            /* check external WAV file name (and modify given file name if required) */
          ...
          break;
        default: PRINTF_OUT("unknown command, try \"help\"\n\n");
          break;
      }
      main_parser_next:
