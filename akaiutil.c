...
int open_disk(char *name, int readonly, OFF64_T startoff, u_int pseudodisksize, u_int pseudodisknum) {
  int fd;
#ifdef _VISUALCPP
                                                                                                                          int fldrn;
	int fldrtype;
  ...
  for (i = 0; (disk_num < DISK_NUM_MAX) && (i < PSEUDODISK_NUM_MAX);) {
    ...
    if ((pseudodisksize == 0) /* pseudo-disk size not given? */
        && ((pseudodisknum == 0)
            || (disksize == 0))) { /* and: max. number of pseudo-disks not given or usable disk size==0? */
      break; /* done */
    }
...
/* Note: possibly not useful for disk with bad blocks */
u_int akai_disksize(int fd, OFF64_T startoff, u_int disksizemax) {
  u_int off, siz;
...
int akai_io_blks(struct part_s *pp, u_char *buf, u_int bstart, u_int bsize, int cachealloc, int mode) {
...
int akai_openreadonly_extfile(char *name) {
  int fd;
  u_int nlen;
  char *nbuf;
  u_int k;
...
int akai_check_extwavname(char *wavname) {
  u_int nlen;
  int fd;
  u_int k;
...
void akai_countfree_part(struct part_s *pp) {
  u_int i;
  u_int n;
...
int akai_check_fatblk(u_int blk, u_int bsize, u_int bsyssize) {
...
int print_fatchain(struct part_s *pp, u_int blk) {
  u_int i;
...
int akai_free_fatchain(struct part_s *pp, u_int bstart, int writeflag) {
  int ret;
  u_int fblk, nblk;
  u_int bc;
  u_int i;
...
int akai_allocate_fatchain(struct part_s *pp, u_int bsize, u_int *bstartp, u_int bcont0, u_int endcode) {
  int ret;
  u_int fblk, pblk;
  u_int bc;
  u_int hdsiz;
...
int akai_scanbad(struct part_s *pp, int markflag) {
...
int akai_read_file(int outfd, u_char *outbuf, struct file_s *fp, u_int begin, u_int end) {
...
int akai_write_file(int inpfd, u_char *inpbuf, struct file_s *fp, u_int begin, u_int end) {
  u_char fbuf[AKAI_HD_BLOCKSIZE];
  u_int fblk, fchunk, fremain, skipbyte;
  u_char (*fatp)[2];
  int err;
...
int print_ddfatchain(struct part_s *pp, u_int cstart) {
  u_int i;
  u_int cl, nextcl;
...
u_int akai_count_ddfatchain(struct part_s *pp, u_int cstart) {
  u_int cc;
  u_int i;
  u_int cl, nextcl;
...
/* Note: if writeflag==0, free block counter of partition is not updated!!! */
int akai_free_ddfatchain(struct part_s *pp, u_int cstart, int writeflag) {
  int ret;
  u_int cl, nextcl;
  u_int cc;
  u_int i;
...
int akai_allocate_ddfatchain(struct part_s *pp, u_int csize, u_int *cstartp, u_int ccont0) {
  int ret;
  u_int fcl, pcl;
  u_int cc;
...
int akai_export_ddfatchain(struct part_s *pp, u_int cstart, u_int bstart, u_int bsize, int outfd, u_char *outbuf) {
...
int akai_import_ddfatchain(struct part_s *pp, u_int cstart, u_int bstart, u_int bsize, int inpfd, u_char *inpbuf) {
  static u_char buf[AKAI_DDPART_CBLKS * AKAI_HD_BLOCKSIZE];
  u_int bufsiz;
  u_int i, i0, i1;
  u_int cl, nextcl;
...
void akai2ascii_name(u_char *aname, char *name, int s900flag) {
  int i;
  int len;
...
void akai_vol_info(struct vol_s *vp, u_int ai, int verbose) {
  ...
  switch (vp->type) {
    case AKAI_VOL_TYPE_INACT: PRINTF_OUT("INACT");
      break;
    case AKAI_VOL_TYPE_S1000: PRINTF_OUT("S1000");
      break;
    case AKAI_VOL_TYPE_S3000: PRINTF_OUT("S3000");
      break;
    case AKAI_VOL_TYPE_CD3000: PRINTF_OUT("CD3000");
      break;
    case AKAI_VOL_TYPE_S900: PRINTF_OUT("S900");
      break;
    default: PRINTF_OUT("(%02x)", vp->type);
      break;
  }
  PRINTF_OUT("\n");
  if (verbose) {
...
int akai_read_voldir(struct vol_s *vp) {
  u_int i, imax;
  u_int blk;
  u_char *addr;
...
int akai_write_voldir(struct vol_s *vp, u_int fi) {
  u_int blk0, blk1;
  u_int blk;
  u_char *addr;
...
void akai_copy_structvol(struct vol_s *srcvp, struct vol_s *dstvp) {
...
/* saves result in *vp */
int akai_get_vol(struct part_s *pp, struct vol_s *vp, u_int vi) {
  u_int i, j;
  ...
  if (pp->type == PART_TYPE_HD) {
    ...
    switch (vp->type) {
      case AKAI_VOL_TYPE_INACT: return -1; /* invalid */
      case AKAI_VOL_TYPE_S1000:
      case AKAI_VOL_TYPE_S3000:
      case AKAI_VOL_TYPE_CD3000: break;
      default:
#if 0
                                                                                                                                /* XXX assume S1000 */
			vp->type=AKAI_VOL_TYPE_S1000;
...
/* saves result in *vp */
int akai_find_vol(struct part_s *pp, struct vol_s *vp, char *name) {
  u_int osver;
  u_int vi;
  u_int ai, aivifound;
...
int akai_rename_vol(struct vol_s *vp, char *name, u_int lnum, u_int osver, struct akai_volparam_s *parp) {
  int modflag;
  u_int blk;
  u_char *addr;
  struct akai_flvol_label_s *lp;
...
/* uses *vp as output */
int akai_create_vol(struct part_s *pp,
                    struct vol_s *vp,
                    u_int type,
                    u_int index,
                    char *name,
...
void akai_list_volparam(struct akai_volparam_s *parp, int listflag) {
  u_int i;
  ...
  if (listflag) {
    ...
    for (i = 0; i < sizeof(struct akai_volparam_s); i++) {
      ...
      switch (i) {
        case 0: PRINTF_OUT("  (program select MIDI channel-1)\n");
          break;
        case 1: PRINTF_OUT("  (MIDI OMNI for program select)\n");
          break;
        case 2: PRINTF_OUT("  (program select enable)\n");
          break;
        case 3: PRINTF_OUT("  (selected program number-1)\n");
          break;
        case 4: PRINTF_OUT("  (MIDI OMNI override for play input)\n");
          break;
        case 5: PRINTF_OUT("  (MIDI system exclusive channel-1)\n");
          break;
        default: PRINTF_OUT("\n");
          break;
      }
...
void akai_init_volparam(struct akai_volparam_s *parp, int s3000flag) {
...
void akai_print_lnum(u_int lnum) {
...
void akai_list_tags(struct part_s *pp) {
  u_int i;
  char namebuf[AKAI_NAME_LEN + 1]; /* +1 for '\0' */
...
int akai_rename_tag(struct part_s *pp, char *name, u_int ti, int wipeflag) {
  u_int i;
  char namebuf[AKAI_NAME_LEN + 1]; /* +1 for '\0' */
...
int copy_tags(struct part_s *srcpp, struct part_s *dstpp) {
...
void akai_print_cdinfo(struct part_s *pp, int verbose) {
  u_int c;
  u_int i;
  u_int fnum;
  u_int bn;
...
int akai_set_cdinfo(struct part_s *pp, char *cdlabel) {
  u_int c;
  u_int i, j;
  u_int fnum, fnummax;
  u_int vsiz;
  ...
  /* XXX minimum size */
  fnummax = ((AKAI_CDINFO_MINSIZB * AKAI_HD_BLOCKSIZE) - sizeof(struct akai_cdinfohead_s))
      / sizeof(struct akai_voldir_entry_s);
...
void akai_part_info(struct part_s *pp, int verbose) {
  ...
  if (verbose) {
    ...
    PRINTF_OUT("parttype:  ");
    switch (pp->type) {
      case PART_TYPE_FLL: PRINTF_OUT("low-density floppy");
        break;
      case PART_TYPE_FLH: PRINTF_OUT("high-density floppy");
        break;
      case PART_TYPE_HD9: PRINTF_OUT("S900 harddisk");
        break;
      case PART_TYPE_HD: PRINTF_OUT("S1000/S3000 harddisk sampler partition");
        break;
      case PART_TYPE_DD: PRINTF_OUT("S1100/S3000 harddisk DD parttion");
        break;
      default: PRINTF_OUT("\?\?\?");
        break;
    }
    ...
    switch (pp->type) {
      case PART_TYPE_FLL: PRINTF_OUT("FLL");
        break;
      case PART_TYPE_FLH: PRINTF_OUT("FLH");
        break;
      case PART_TYPE_HD9: PRINTF_OUT("HD9");
        break;
      case PART_TYPE_HD: PRINTF_OUT(" HD");
        break;
      case PART_TYPE_DD: PRINTF_OUT(" DD");
        break;
      default: PRINTF_OUT("\?\?\?");
        break;
    }
...
void print_fat(struct part_s *pp) {
  ...
  for (i = 0; i < ilim; i++) {
    ...
    if (pp->type == PART_TYPE_DD) { /* harddisk DD partition? */
      switch (j) {
        case AKAI_DDFAT_CODE_FREE: PRINTF_OUT(" -   ");
          break;
        case AKAI_DDFAT_CODE_BAD: PRINTF_OUT(" BAD ");
          break;
        case AKAI_DDFAT_CODE_SYS: PRINTF_OUT(" SYS ");
          break;
        case AKAI_DDFAT_CODE_END: PRINTF_OUT(" END ");
          break;
        default: PRINTF_OUT(" %04x", j);
          break;
      }
    } else {
      if (i < pp->bsyssize) { /* reserved block for system? */
        ...
        switch (j) {
          case AKAI_FAT_CODE_FREE: /* XXX same as AKAI_FAT_CODE_SYS900FL */
            PRINTF_OUT(" -   ");
            break;
          case AKAI_FAT_CODE_BAD: PRINTF_OUT(" BAD ");
            break;
          ...
          case AKAI_FAT_CODE_FILEEND: PRINTF_OUT(" END ");
            break;
          default: PRINTF_OUT(" %04x", j);
            break;
        }
...
void akai_disk_info(struct disk_s *dp, int verbose) {
  ...
  if (verbose) {
    ...
    PRINTF_OUT("disktype: ");
    switch (dp->type) {
      case DISK_TYPE_FLL: PRINTF_OUT("low-density floppy");
        break;
      case DISK_TYPE_FLH: PRINTF_OUT("high-density floppy");
        break;
      case DISK_TYPE_HD9: PRINTF_OUT("S900 harddisk");
        break;
      case DISK_TYPE_HD: PRINTF_OUT("S1000/S3000 harddisk");
        break;
      default: PRINTF_OUT("\?\?\?");
        break;
    }
    ...
    switch (dp->type) {
      case DISK_TYPE_FLL: PRINTF_OUT("FLL");
        break;
      case DISK_TYPE_FLH: PRINTF_OUT("FLH");
        break;
      case DISK_TYPE_HD9: PRINTF_OUT("HD9");
        break;
      case DISK_TYPE_HD: PRINTF_OUT(" HD");
        break;
      default: PRINTF_OUT("\?\?\?");
        break;
    }
...
void akai_list_disk(struct disk_s *dp, int recflag, u_char *filtertagp) {
  u_int p;
  u_int totp;
...
void akai_list_alldisks(int recflag, u_char *filtertagp) {
  u_int di;
...
void akai_sort_filetags(u_char *tagp) {
  u_int i, j;
  u_char b;
  ...
  // TODO(tdial): Look into whether it's worth using a better sort.
...
int akai_match_filetags(u_char *filtertagp, u_char *testtagp) {
  u_int i, j;
...
int akai_set_filetag(u_char *tagp, u_int ti) {
  u_int i;
...
int akai_clear_filetag(u_char *tagp, u_int ti) {
  u_int i;
...
/* saves result in *fp */
int akai_get_file(struct vol_s *vp, struct file_s *fp, u_int fi) {
...
/* saves result in *fp */
int akai_find_file(struct vol_s *vp, struct file_s *fp, char *name) {
  u_int fi;
  char namebuf[AKAI_NAME_LEN + 4 + 1]; /* +4 for ".<type>", +1 for '\0' */
  u_int i, j, k, l;
...
int akai_rename_file(struct file_s *fp, char *name, struct vol_s *vp, u_int dstindex, u_char *tagp, u_int osver) {
  u_int i;
  u_int diffvolflag;
  u_int fisrc, fidst;
  ...
  if ((name == NULL)
      && ((osver != fp->osver) || diffvolflag)) { /* no new name but different OS version or different volume? */
...
/* uses *fp as output */
/* size in bytes */
int akai_create_file(struct vol_s *vp,
                     struct file_s *fp,
                     u_int size,
                     u_int index,
                     char *name,
  ...
  if (akai_allocate_fatchain(vp->partp, bsize,
                             &fp->bstart,
                             1,
                             (vp->type == AKAI_VOL_TYPE_S900) ? AKAI_FAT_CODE_FILEEND900 : AKAI_FAT_CODE_FILEEND)
      < 0) { /* end code */
    return -1;
  }
...
void akai_fvol1000_initfile(struct akai_voldir_entry_s *ep, u_int osver, u_int tag) {
  u_int i;
...
int akai_delete_file(struct file_s *fp) {
  u_int fi;
...
int akai_rename_ddtake(struct part_s *pp, u_int ti, char *name) {
...
int akai_delete_ddtake(struct part_s *pp, u_int ti) {
...
void akai_fix_partheadmagic(struct part_s *pp) {
  u_int i;
  u_int m, cs;
...
int akai_check_partheadmagic(struct part_s *pp) {
  u_int i;
  u_int m, cs, c;
...
void akai_fix_parttabmagic(struct akai_parttab_s *ptp) {
  u_int i;
  u_int m;
...
int akai_check_parttabmagic(struct akai_parttab_s *ptp) {
  u_int i;
  u_int m;
...
int akai_scan_floppy(struct disk_s *dp) {
  u_int i, n;
  ...
  if (i > 0) {
    ...
    if ((dp->bsize >= AKAI_FLL_SIZE) && (dp->bsize < AKAI_FLH_SIZE)
        && ((i == AKAI_FLLHEAD_BLKS)
            || (i == (AKAI_FLLHEAD_BLKS + AKAI_VOLDIR3000FL_BLKS)))) { /* S1000 or S3000 low-density? */
      ...
      /* Note: see comment above: no need to read low-density header again */
        ...
        && ((i == AKAI_FLHHEAD_BLKS)
            || (i == (AKAI_FLHHEAD_BLKS + AKAI_VOLDIR3000FL_BLKS)))) { /* S1000 or S3000 high-density? */
...
int akai_scan_harddisk9(struct disk_s *dp) {
  u_int i, n;
...
int akai_scan_ddpart(struct disk_s *dp, struct akai_parttab_s *ptp, u_int bstart, u_int pi) {
  u_int ptcsize;
  u_int ptbsize;
  u_int totsize;
  u_int di, dimax;
...
int akai_scan_disk(struct disk_s *dp, int floppyenable) {
  u_int bstart;
  u_int pi, pimax;
  u_int ptbsize;
...
int akai_wipe_part(struct part_s *pp, int wipeflag, struct part_s *plistp, u_int pimax, int cdromflag) {
  int i;
  char namebuf[AKAI_NAME_LEN + 1]; /* +1 for '\0' */
  u_int pi, siz;
  u_int pnum, totb;
...
int akai_wipe_harddisk(struct disk_s *dp, u_int bsize, u_int totb, int s3000flag, int cdromflag) {
  u_int pi, pimax;
...
void curdir_info(int verbose, char *pathbuf) {
...
int copy_file(struct file_s *srcfp,
              struct vol_s *dstvp,
              struct file_s *dstfp,
              u_int dstindex,
              char *dstname,
...
int copy_vol_allfiles(struct vol_s *srcvp, struct part_s *dstpp, char *dstname, int delflag, int verbose) {
  struct vol_s tmpvol;
  struct file_s tmpfile;
  u_int fi;
...
int copy_part_allvols(struct part_s *srcpp, struct part_s *dstpp, int delflag, int verbose) {
  u_int vi;
  struct vol_s tmpvol;
...
int check_curnosamplervol(void) {
...
int check_curnosamplerpart(void) {
