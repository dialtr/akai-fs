...
/* AKAI file info */
int akai_file_info(struct file_s *fp, int verbose) {
  // TODO(tdial): Static
...
void akai_sample_info(struct file_s *fp, u_char *hdrp) {
  ...
  if (fp->type == AKAI_SAMPLE900_FTYPE) {
    ...
    switch (s900hdrp->pmode) {
      case SAMPLE900_PMODE_ONESHOT: PRINTF_OUT("ONESHOT\n");
        break;
      case SAMPLE900_PMODE_LOOP: PRINTF_OUT("LOOP\n");
        break;
      case SAMPLE900_PMODE_ALTLOOP: PRINTF_OUT("ALTLOOP\n");
        break;
      default: PRINTF_OUT("\?\?\?\n");
        break;
    }
    PRINTF_OUT("dir:     ");
    switch (s900hdrp->dir) {
      case SAMPLE900_DIR_NORM: PRINTF_OUT("NORM\n");
        break;
      case SAMPLE900_DIR_REV: PRINTF_OUT("REV\n");
        break;
      default: PRINTF_OUT("\?\?\?\n");
        break;
    }
    ...
    switch (s900hdrp->type) {
      case SAMPLE900_TYPE_NORM: PRINTF_OUT("NORM\n");
        break;
      case SAMPLE900_TYPE_VELXF: PRINTF_OUT("VELXF\n");
        break;
      default: PRINTF_OUT("\?\?\?\n");
        break;
    }
    ...
    switch (s3000hdrp->s1000.bandw) {
      case SAMPLE1000_BANDW_10KHZ: PRINTF_OUT("10kHz\n");
        break;
      case SAMPLE1000_BANDW_20KHZ: PRINTF_OUT("20kHz\n");
        break;
      default: PRINTF_OUT("\?\?\?\n");
        break;
    }
    ...
    switch (s3000hdrp->s1000.pmode) {
      case SAMPLE1000_PMODE_LOOP: PRINTF_OUT("LOOP\n");
        break;
      case SAMPLE1000_PMODE_LOOPNOTREL: PRINTF_OUT("LOOPNOTREL\n");
        break;
      case SAMPLE1000_PMODE_NOLOOP: PRINTF_OUT("NOLOOP\n");
        break;
      case SAMPLE1000_PMODE_TOEND: PRINTF_OUT("TOEND\n");
        break;
      default: PRINTF_OUT("\?\?\?\n");
        break;
    }
...
int akai_program_info(struct file_s *fp) {
  static char nbuf[AKAI_NAME_LEN + 1]; /* +1 for '\0' */
  ...
  if (fp->type == AKAI_PROGRAM900_FTYPE) {
    ...
    for (i = 0; i < kgnum; i++) {
      ...
      switch (s900kgp->outch1) {
        case PROGRAM900KG_OUTCH1_LEFT: PRINTF_OUT("LEFT\n");
          break;
        case PROGRAM900KG_OUTCH1_RIGHT: PRINTF_OUT("RIGHT\n");
          break;
        case PROGRAM900KG_OUTCH1_ANY: PRINTF_OUT("ANY\n");
          break;
        default: PRINTF_OUT("%u\n", s900kgp->outch1 + 1);
          break;
      }
    ...
    for (i = 0; i < kgnum; i++) {
      ...
      for (j = 0; j < PROGRAM1000KG_VELZONENUM; j++) {
        ...
        switch (s3000kgp->s1000.velzone[j].pmode) {
          case PROGRAM1000_PMODE_SAMPLE: PRINTF_OUT("SAMPLE\n");
            break;
          case PROGRAM1000_PMODE_LOOP: PRINTF_OUT("LOOP\n");
            break;
          case PROGRAM1000_PMODE_LOOPNOTREL: PRINTF_OUT("LOOPNOTREL\n");
            break;
          case PROGRAM1000_PMODE_NOLOOP: PRINTF_OUT("NOLOOP\n");
            break;
          case PROGRAM1000_PMODE_TOEND: PRINTF_OUT("TOEND\n");
            break;
          default: PRINTF_OUT("\?\?\?\n");
            break;
        }
...
int akai_cuelist_info(struct file_s *fp) {
  static char nbuf[AKAI_NAME_LEN + 1]; /* +1 for '\0' */
  ...
  if (cuenum > 0) {
    ...
    for (i = 0; i < cuenum; i++) {
      ...
      switch (cuep->type) {
        case AKAI_CUETYPE_MOFF: tname = "MOFF";
          break;
        case AKAI_CUETYPE_MON: tname = "MON ";
          break;
        case AKAI_CUETYPE_TOFF: tname = "TOFF";
          break;
        case AKAI_CUETYPE_TON: tname = "TON ";
          break;
        default: tname = "\?\?\? ";
          break;
      }
...
int akai_takelist_info(struct file_s *fp) {
  static char nbuf[AKAI_NAME_LEN + 1]; /* +1 for '\0' */
  struct akai_cuelist_s *hdrp;
  struct akai_cue_s *cuep;
  u_char *buf;
...
int akai_drum1000_info(struct file_s *fp) {
...
int akai_drum900_info(struct file_s *fp) {
...
int akai_ovs900_info(struct file_s *fp) {
  ...
  switch (ofp->ctrlport) {
    case OVS900_CTRLPORT_MIDI: PRINTF_OUT("MIDI\n");
      break;
    case OVS900_CTRLPORT_RS232: PRINTF_OUT("RS232\n");
      break;
    default: PRINTF_OUT("\?\?\?\n");
      break;
  }
...
int akai_cdsetup3000_info(struct file_s *fp) {
...
int akai_fixramname(struct file_s *fp) {
  static char nbuf[AKAI_NAME_LEN + 1]; /* +1 for '\0' */
...
int akai_s900comprfile_updateuncompr(struct file_s *fp) {
  u_int osver;
...
u_int akai_sample900_getsamplesize(struct file_s *fp) {
...
void akai_sample900noncompr_sample2wav(u_char *sbuf, u_char *wavbuf, u_int samplecountpart) {
  u_int i;
...
void akai_sample900noncompr_wav2sample(u_char *sbuf, u_char *wavbuf, u_int samplecountpart) {
  u_int i;
...
u_int akai_sample900compr_getbits(u_char *buf, u_int bitpos, u_int bitnum) {
  u_int bytepos;
  u_char bmask;
  u_int val;
  u_int i;
...
u_int akai_sample900compr_sample2wav(u_char *sbuf, u_char *wavbuf, u_int sbufsiz, u_int wavbufsiz) {
  u_char upbitnum;
  u_char upsign;
  u_short upabsval;
  short curval;
  ...
  for (; (bitremain > 0) && (wavpos + 1 < wavbufsiz);) {
    ...
    if (code == 0x0) {
      ...
      for (i = 0; (i < SAMPLE900COMPR_GROUP_SAMPNUM) && (wavpos + 1 < wavbufsiz); i++) {
        ...
        /* get value word */
        upabsval = (u_short) akai_sample900compr_getbits(sbuf,
                                                         bitpos + SAMPLE900COMPR_GROUP_SAMPNUM + i * ((u_int) upbitnum),
                                                         (u_int) upbitnum);
#ifdef SAMPLE900COMPR_DEBUG
...
void akai_sample900compr_setbits(u_char *buf, u_int bitpos, u_int bitnum, u_int val) {
  u_int bytepos;
  u_char bmask;
  u_int i;
...
int akai_sample900compr_wav2sample(u_char *sbuf, u_char *wavbuf, u_int samplecountpart) {
  ...
  if (sbuf == NULL) {
    ...
    groupcount =
        (2 * samplecountpart + 1 + SAMPLE900COMPR_GROUP_SAMPNUM - 1) / SAMPLE900COMPR_GROUP_SAMPNUM; /* round up */
...
akai_sample900_compr2noncompr(struct file_s *fp, struct vol_s *volp) {
  ...
  if (akai_read_file(0,
                     sbufcompr,
                     fp,
                     sizeof(struct akai_sample900_s),
                     sizeof(struct akai_sample900_s) + samplesizecompr) < 0) {
    PRINTF_ERR("cannot read sample\n");
    goto akai_sample900_compr2noncompr_exit;
  }
  ...
  if (akai_write_file(0,
                      sbufnoncompr,
                      &tmpfile,
                      sizeof(struct akai_sample900_s),
                      sizeof(struct akai_sample900_s) + samplesizenoncompr) < 0) {
...
akai_sample2wav(struct file_s *fp, int wavfd, u_int *sizep, char **wavnamep, int what) {
  ...
  if (what & SAMPLE2WAV_EXPORT) {
...
#ifndef WAV_AKAIHEAD_DISABLE
    {
      ...
      if (WRITE(wavfd, (u_char *) &wavchunkhead, sizeof(struct wav_chunkhead_s))
...
int akai_wav2sample(int wavfd, char *wavname, struct vol_s *volp, u_int findex,
